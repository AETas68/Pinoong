...
module.exports = function (io) {
  ...
  router.post('/save-dish', requireAuth, async (req, res) => {
    ...
    try {
      ...
      // ---- Nạp ngay món BTP này vào Danh Mục NVL (nhóm 🍲 Nhóm BTP) ----
      // Trước đây chỉ tạo dòng NVL khi bấm "Sản Xuất Mẻ" — khiến món BTP mới
      // lưu công thức xong vẫn không chọn được làm nguyên liệu ở Menu, và
      // trang NVL báo "chưa sản xuất". Giờ CHỈ CẦN lưu công thức là đã có
      // dòng NVL tương ứng ngay (giá tạm tính theo công thức + SL danh nghĩa
      // — sẽ được ghi đè bằng giá THẬT chính xác hơn khi "Sản Xuất Mẻ").
      if (oldName && normName(oldName) !== normName(dish.name)) {
        // Đổi tên món -> đổi tên luôn dòng NVL tương ứng (nếu có, và tên mới
        // chưa bị dòng NVL nào khác chiếm).
      ...
      let btpNvlRow = data.nvl.find(n => normName(n.ten) === normName(dish.name));
      if (!btpNvlRow) {
        const { totalNvl } = calcRecipeCost(dish, data.nvl);
        const estimatedGia = dish.output_qty > 0 ? +(totalNvl / dish.output_qty).toFixed(2) : 0;
        const maxId = Math.max(0, ...data.nvl.map(n => n.id || 0));
        btpNvlRow = { id: maxId + 1, ten: dish.name, dvt: dish.output_unit, gia: estimatedGia, nhom: '🍲 Nhóm BTP' };
        data.nvl.push(btpNvlRow);
      } else {
        // Đã có dòng NVL (từ trước hoặc từ lần Sản Xuất Mẻ trước) -> chỉ cập
        // nhật ĐVT cho khớp, KHÔNG đụng vào giá (giá THẬT từ lần sản xuất gần
        // nhất đáng tin hơn giá ước tính theo công thức danh nghĩa).
        btpNvlRow.dvt = dish.output_unit;
      }
  ...
  router.delete('/dish/:id', requireAuth, async (req, res) => {
    ...
    try {
      ...
      if (data.btp_recipes?.[branch]) {
        const target = data.btp_recipes[branch].find(d => d.id === id);
        if (target) deletedName = target.name;
        data.btp_recipes[branch] = data.btp_recipes[branch].filter(d => d.id !== id);
      }
      // Nếu không còn chi nhánh nào (Khạp Khun / Pinoong) dùng tên món này
      // nữa -> đưa dòng NVL tương ứng ra khỏi 🍲 Nhóm BTP (không xoá NVL,
      // chỉ đổi nhóm, để không phá vỡ giá vốn Menu đã tính trước đó).
      if (deletedName && Array.isArray(data.nvl)) {
        const stillUsed = ['khapkhun', 'pinoong'].some(b =>
          (data.btp_recipes?.[b] || []).some(d => normName(d.name) === normName(deletedName))
        );
        if (!stillUsed) {
          const nvlRow = data.nvl.find(n => normName(n.ten) === normName(deletedName) && n.nhom === '🍲 Nhóm BTP');
          if (nvlRow) nvlRow.nhom = '📦 Nhóm Hàng Khô';
        }
      }
      await saveState(client, data, req.user.name, req.headers['x-socket-id']);
      await client.query('COMMIT');
