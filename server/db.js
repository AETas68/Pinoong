...
// Đưa mọi NVL đang gắn nhóm "🍲 Nhóm BTP" nhưng KHÔNG tương ứng với bất kỳ
// công thức nào trong data.btp_recipes.khapkhun / .pinoong -> nhóm
// "📦 Nhóm Hàng Khô". Ngược lại, không tự ý gắn thêm gì vào nhóm BTP ở đây —
// việc đó do server/btp.js làm ngay khi lưu công thức (POST /save-dish).
async function syncNhomBtpTrongNvl() {
  ...
  try {
    ...
    const raw = rows[0].data;
    const data = typeof raw === 'string' ? JSON.parse(raw) : (raw || {});
    if (!Array.isArray(data.nvl)) { await client.query('ROLLBACK'); return; }

    const validNames = new Set();
    for (const branch of ['khapkhun', 'pinoong']) {
      for (const d of (data.btp_recipes?.[branch] || [])) {
        validNames.add(normName(d.name));
      }
    }
    ...
    for (const n of data.nvl) {
      if (n.nhom === '🍲 Nhóm BTP' && !validNames.has(normName(n.ten))) {
        n.nhom = '📦 Nhóm Hàng Khô';
        movedNames.push(n.ten);
        moved++;
      }
    }
    ...
    if (moved > 0) {
      ...
      console.log(`🧹 Đã đưa ${moved} NVL ra khỏi Nhóm BTP (không có công thức ở file BTP nào): ${movedNames.join(', ')}`);
    }
