...
async function initSchema() {
  ...
  // ------------------------------------------------------------------
  // 3. BANG nvl_kho — nguồn NGUYÊN VẬT LIỆU DÙNG CHUNG duy nhất
  //    (2 file BTP sẽ cùng đọc / cùng trừ tồn từ bảng này)
  // ------------------------------------------------------------------
  await pool.query(`
    CREATE TABLE IF NOT EXISTS nvl_kho (
      id SERIAL PRIMARY KEY,
  ...
  // ------------------------------------------------------------------
  // 4. BANG btp_recipes — công thức BTP, có cột branch để tách theo quán,
  //    nhưng nằm CHUNG một bảng (để báo cáo tổng dễ gộp lại).
  // ------------------------------------------------------------------
  await pool.query(`
    CREATE TABLE IF NOT EXISTS btp_recipes (
      id SERIAL PRIMARY KEY,
  ...
  // ------------------------------------------------------------------
  // 5. BANG btp_export_logs — nhật ký xuất bán BTP theo từng quán,
  //    dùng để gộp báo cáo P&L (doanh thu / chi phí) chung toàn hệ thống.
  // ------------------------------------------------------------------
  await pool.query(`
    CREATE TABLE IF NOT EXISTS btp_export_logs (
      id SERIAL PRIMARY KEY,
      branch TEXT NOT NULL,
      dish_name TEXT,
      qty NUMERIC,
      unit TEXT,
      von_nvl NUMERIC DEFAULT 0,
      doanh_thu NUMERIC DEFAULT 0,
      ln NUMERIC DEFAULT 0,
      note TEXT,
      created_at TIMESTAMPTZ DEFAULT now()
    );
  `);
  ...
  // ------------------------------------------------------------------
  // 6. BANG btp_branch_auth — mật khẩu RIÊNG cho từng file BTP,
  ...
  // ------------------------------------------------------------------
  // 7. Seed tài khoản quản lý đầu tiên NẾU bảng users đang trống hẳn
