 ...
 * người nhận không có quyền BTP tương ứng — dù họ có mở Console trình duyệt
 * cũng không lấy được, vì dữ liệu chưa từng được gửi xuống máy họ.
 *   - data.btp_recipes / data.btp_recipes_deleted: lọc riêng theo TỪNG chi nhánh.
 *   - data.btp_production: mỗi dòng có field "ten" (NVL thô) + "branch" ->
 *     lọc bỏ hẳn các dòng thuộc chi nhánh không có quyền.
 *   - data.btp_raw_expansion: bảng quy đổi GỘP CHUNG cả 2 chi nhánh (không
 *     tách được theo từng nhánh — xem resolveBtpRawExpansion ở trên), nên hễ
 *     thiếu quyền BẤT KỲ chi nhánh nào thì ẩn hẳn toàn bộ bảng cho an toàn.
...
function filterBtpSecrets(data, user) {
  ...
  const canPN = perm.includes('btp-pinoong');
  if (canKK && canPN) return data; // đủ quyền cả 2 nhánh -> không cần lọc gì

  const out = { ...data };
  if (out.btp_recipes) {
    out.btp_recipes = {
      khapkhun: canKK ? (out.btp_recipes.khapkhun || []) : [],
      pinoong: canPN ? (out.btp_recipes.pinoong || []) : []
    };
  }
  if (out.btp_recipes_deleted) {
    out.btp_recipes_deleted = {
      khapkhun: canKK ? (out.btp_recipes_deleted.khapkhun || []) : [],
      pinoong: canPN ? (out.btp_recipes_deleted.pinoong || []) : []
    };
  }
  if (out.btp_production) {
    const filtered = {};
    for (const mk of Object.keys(out.btp_production)) {
      filtered[mk] = (out.btp_production[mk] || []).filter(e =>
        e.branch === 'khapkhun' ? canKK : e.branch === 'pinoong' ? canPN : true
      );
    }
    out.btp_production = filtered;
  }
  out.btp_raw_expansion = {}; // gộp chung 2 nhánh, không tách được -> ẩn hẳn nếu thiếu quyền 1 trong 2
